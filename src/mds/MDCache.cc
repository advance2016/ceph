...
void MDCache::dispatch_fragment_dir(const MDRequestRef& mdr, bool abort_if_freezing)
{
  ...
  if (!mdr->aborted && !(mdr->locking_state & MutationImpl::ALL_LOCKED)) {
    MutationImpl::LockOpVec lov;
    // a freezing dir can't be quiesced
    ceph_assert(!diri->is_quiesced());
...
void MDCache::quiesce_overdrive_fragmenting(CDir* dir) {
...
void MDCache::dispatch_quiesce_inode(const MDRequestRef& mdr)
{
  ...
  if (!(mdr->locking_state & MutationImpl::ALL_LOCKED)) {
    ...
    if (!mds->locker->acquire_locks(mdr, lov)) {
      quiesce_overdrive_fragmenting(in->get_parent_dir());
      return;
    }
    mdr->locking_state |= MutationImpl::ALL_LOCKED;
...
void MDCache::dispatch_quiesce_path(const MDRequestRef& mdr)
{
  ...
  *qis = {mdr, qfinisher->qs, delay, splitauth};
  ...
  if (r > 0) {
    quiesce_overdrive_fragmenting(curdir);
    return;
  } else if (r < 0) {
    mds->server->respond_to_request(mdr, r);
    return;
